...
export function ProjectsPage() {
  const { activeOrganization, isLoading: isLoadingOrganizations, error: organizationError } = useOrganizations();
  const { projects, stats, isLoading, error, pauseProject, archiveProject } = useProjects(activeOrganization?.id);
  ...
  const filteredProjects = useMemo(() => {
    ...
    return projects.filter((project) => {
      const matchesSearch =
        !normalizedSearch ||
        [project.name, project.description, projectTypeLabels[project.projectType], environmentLabels[project.environment], statusLabels[project.status], project.connectionMode]
          .filter(Boolean)
    ...
    <div className="min-w-0 space-y-6 pb-10 sm:space-y-8">
      ...
      <header className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        ...
        <Link
          ...
          to={activeOrganization ? "/projects/new" : appRoutes.onboardingOrganization}
      ...
      {isLoading || isLoadingOrganizations ? <div className="rounded-lg border border-[#dce2ea] bg-white px-8 py-12 text-center font-semibold text-[#64708a]">Cargando proyectos...</div> : <ProjectTable projects={filteredProjects} onPauseProject={pauseProject} onArchiveProject={archiveProject} />}
    </div>
  );
}
