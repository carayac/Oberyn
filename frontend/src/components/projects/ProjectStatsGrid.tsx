...
const statCards = [
  { id: "active-projects", label: "Proyectos activos", key: "activeProjects", help: "Total real", Icon: Folder, className: "bg-emerald-50 text-emerald-700" },
  { id: "connected-apis", label: "APIs conectadas", key: "connectedApis", help: "Registradas en BD", Icon: Plug, className: "bg-sky-50 text-sky-700" },
  { id: "active-rules", label: "Reglas activas", key: "activeRules", help: "Configuradas", Icon: ShieldCheck, className: "bg-violet-50 text-violet-700" },
  { id: "allowed-flows", label: "Flujos permitidos", key: "allowedFlows", help: "Permitidos", Icon: Repeat2, className: "bg-amber-50 text-amber-700" },
] as const;
...
export function ProjectStatsGrid({ stats }: ProjectStatsGridProps) {
  return (
    <section className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(min(100%,240px),1fr))]">
      {statCards.map(({ id, label, key, help, Icon, className }) => (
        <article key={id} id={`project-stat-${id}`} className="flex min-h-[112px] min-w-0 items-center gap-4 rounded-lg border border-[#dce2ea] bg-white px-5 py-5 shadow-[0_10px_28px_rgba(15,23,42,0.04)]">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm font-semibold text-[#596783]">{help}</p>
          </div>
        </article>
      ))}
    </section>
